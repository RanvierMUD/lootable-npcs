...
const { Logger, RandomUtil: Random } = require('ranvier');
...
class LootTable {
  ...
  async load(state) {
    this.pools = this.pools
      .map(async pool => await this.resolvePool(state, pool))
  ...
  async resolvePool(state, pool) {
    ...
    if (!Array.isArray(resolvedPool)) {
      ...
      pool = resolvedPool
        .map(async nestedPool => await this.resolvePool(state, nestedPool))
        .reduce((acc, val) => acc.concat(val), [])
      ;
    }
